...
Sub RenameUploadedFiles(REUF_path,REUF_renameMask)
  Dim REUF_keys, REUF_i, REUF_curKey, REUF_fileName, REUF_fileNameArr, REUF_lastPos
  Dim REUF_fso, REUF_curPath, REUF_curName, REUF_curExt, REUF_newFileName
  ...
  for REUF_i = 0 to UploadRequest.Count - 1
    ...
    if UploadRequest.Exists(REUF_curKey) then
      if UploadRequest.Item(REUF_curKey).Exists("FileName") then    
        REUF_fileName = UploadRequest.Item(REUF_curKey).Item("FileName")
        if REUF_fileName <> "" then
          ...
          if REUF_curPath = "" then REUF_curPath = REUF_path
            ...
            if REUF_fso.FileExists(Server.MapPath(REUF_curPath & REUF_fileName)) then
...
Sub MailUploadedFiles(MUF_path,MUF_FromName,MUF_FromEmail,MUF_ToName,MUF_ToEmail,MUF_Bcc,MUF_Subject,MUF_Body,MUF_SendHtml,MUF_checkErrors,MUF_RedirectURL,MUF_SmtpServer,MUF_MailerType)
