...
rsTypeListing.ActiveConnection = MM_connDUpaypal_STRING
rsTypeListing.Source = "SELECT * FROM qryInventory,  TYPES  WHERE PRO_TYPE = " + Replace(rsTypeListing__MMColParam, "'", "''") + " AND PRO_TYPE = TYPE_ID  ORDER BY PRO_YEAR, PRO_BRAND, PRO_SUB, PRO_SET, PRO_CARDNUMBER ASC"
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="2">
    ...
    <tr>
      <td align="left" valign="middle" class="textBold"><a href="default.asp">SHOP</a>
        &raquo; <a href="type.asp?iType=<%=(rsTypeListing.Fields.Item("PRO_TYPE").Value)%>"><%=UCASE(rsTypeListing.Fields.Item("TYPE_NAME").Value)%></a> &raquo; LISTING</td>
...
%>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <form target="paypal" action="https://www.paypal.com/cgi-bin/webscr" method="post"><td width="20" align="left">
		                                    ...
		                                    <input type="hidden" name="item_name" value="<%=(rsTypeListing.Fields.Item("PRO_NAME").Value)%>">
		                                    <input type="hidden" name="item_number" value="<%=(rsTypeListing.Fields.Item("PRO_ID").Value)%>">
		                                    <input type="hidden" name="amount" value="<%=(rsTypeListing.Fields.Item("PRO_PRICE").Value)%>">
		                                    <input type="hidden" name="shipping" value="<%=(rsTypeListing.Fields.Item("PRO_SHIP").Value)%>">
		                                    <input type="hidden" name="add" value="1">
		                                    <input name="addtocart" type="submit" class="form" value="Add Cart">
                               </td> </form>
            <td align="left" valign="top"> <table width="100%" border="0" cellpadding="1" cellspacing="1">
                <tr align="left" valign="middle">
                  <td align="center" valign="top"><a href="detail.asp?iPro=<%=(rsTypeListing.Fields.Item("PRO_ID").Value)%>&iType=<%=(rsTypeListing.Fields.Item("PRO_TYPE").Value)%>"></a></td>
                  <td valign="top">
                  	<table width="100%" border="0" cellspacing="2" cellpadding="2">
                      <tr valign="top">
                        <td width="65%" align="left" class="textBold"><b></b>
                          <a href="detail.asp?iPro=<%=(rsTypeListing.Fields.Item("PRO_ID").Value)%>&iType=<%=(rsTypeListing.Fields.Item("PRO_TYPE").Value)%>"><%=(rsTypeListing.Fields.Item("PRO_TITLE").Value)%></a>&nbsp;</td>
                        <td width="15%" align="left" class="text"><strong>#:</strong>
                          <%=(rsTypeListing.Fields.Item("PRO_CARDNUMBER").Value)%></td>
                        <td width="15%" align="left" class="text"><strong>Price:</strong>
                          <%= myPaypalCurrencySign %><%=(rsTypeListing.Fields.Item("PRO_PRICE").Value)%></td>
                        <td width="5%" align="left" class="text"><a href="detail.asp?iPro=<%=(rsTypeListing.Fields.Item("PRO_ID").Value)%>&iType=<%=(rsTypeListing.Fields.Item("PRO_TYPE").Value)%>">more</a></td>
                      </tr>
                    </table></td>
                </tr>
              </table></td>
